...
RUN     echo "======================================== " && \
        ...
        if [ -n "${LOCAL_GSTREAMER_FILE}" ]; then \
            LOCAL_VERSION="$(basename "${LOCAL_GSTREAMER_FILE}" | sed -n 's/^gstreamer-selkies_gpl_v\\([^_]*\\)_ubuntu.*$/\\1/p')"; \
        elif [ -n "${LOCAL_WHL_FILE}" ]; then \
            LOCAL_VERSION="$(basename "${LOCAL_WHL_FILE}" | sed -n 's/^selkies_gstreamer-\\(.*\\)-py3-none-any\\.whl$/\\1/p')"; \
        elif [ -n "${LOCAL_WEB_FILE}" ]; then \
            LOCAL_VERSION="$(basename "${LOCAL_WEB_FILE}" | sed -n 's/^selkies-gstreamer-web_v\\(.*\\)\\.tar\\.gz$/\\1/p')"; \
        elif [ -n "${LOCAL_JS_DEB_FILE}" ]; then \
            LOCAL_VERSION="$(basename "${LOCAL_JS_DEB_FILE}" | sed -n 's/^selkies-js-interposer_v\\([^_]*\\)_ubuntu.*$/\\1/p')"; \
        fi && \
        if [ -z "${SELKIES_LOCAL_DIR}" ]; then \
            ...
            MISSING_LOCAL="" && \
            if [ -z "${LOCAL_GSTREAMER_FILE}" ]; then MISSING_LOCAL="${MISSING_LOCAL} gstreamer-selkies_gpl"; fi && \
        ...
        if [ "${USE_LOCAL}" = "true" ]; then \
            ...
            if [ -n "${LOCAL_VERSION}" ]; then \
                ...
                echo "? Using Selkies version from local artifacts: ${SELKIES_VERSION}"; \
            else \
        ...
        rm -rf /tmp/selkies && \
        apt-get clean && rm -rf /var/lib/apt/lists/* /var/cache/debconf/* /var/log/* /tmp/* /var/tmp/*# Copy and extract Warplay driver from host (optional)
